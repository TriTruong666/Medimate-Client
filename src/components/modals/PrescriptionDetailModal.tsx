      ...
      {open && (
        <div className="fixed inset-0 z-120 flex items-center justify-center bg-black/80 p-4 backdrop-blur-sm">
          ...
          <motion.div
            ...
            className="z-10 flex h-[90vh] max-h-215 min-h-0 w-full max-w-5xl flex-col overflow-hidden rounded-2xl border border-white/10 bg-neutral-900/90 shadow-2xl"
          >
            <div className="flex items-center justify-between border-b border-white/10 bg-white/5 p-4 md:px-6">
              <div>
                <h2 className="text-lg font-semibold text-white">Chi tiết đơn thuốc</h2>
              ...
              {isLoading ? (
                ...
                <div className="flex min-h-90 flex-col items-center justify-center rounded-2xl border border-white/10 bg-white/5 p-6 text-center">
                  ...
                  <button
                    ...
                    className="bg-primary mt-6 rounded-lg px-5 py-2.5 text-sm font-medium text-white transition hover:brightness-110"
                  >
                    Thử lại
                  </button>
                </div>
                ...
                <div className="grid gap-4 lg:grid-cols-3">
                  <div className="space-y-4 lg:col-span-2">
                    <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <h3 className="text-base font-semibold text-white">Thông tin cơ bản</h3>
                    ...
                    <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <h3 className="text-base font-semibold text-white">Chẩn đoán và lời dặn</h3>
                      <p className="mt-3 text-sm text-white">{data.diagnosis}</p>
                    ...
                    <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <h3 className="text-base font-semibold text-white">Danh sách thuốc</h3>
                      <div className="mt-4 space-y-3">
                        {data.medicines.map((medicine, index) => (
                          <div key={index} className="rounded-lg border border-white/10 bg-white/5 p-4">
                            ...
                            <p className="mt-1 text-xs text-white/70">{medicine.instructions}</p>
                          </div>
                        ))}
                      </div>
                    </section>
                  ...
                  <div className="space-y-4">
                    <section className="rounded-2xl border border-white/10 bg-white/5 p-5">
                      <h4 className="text-sm font-semibold text-white">Tóm tắt</h4>
                      <div className="mt-4 space-y-3">
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-white/50">Trạng thái</span>
                          <Badge type="info" value={data.status || "Active"} />
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-white/50">Số thuốc</span>
                          <span className="text-sm font-medium text-white">{data.medicines.length}</span>
                        </div>
                        <div className="flex items-center justify-between">
                          <span className="text-xs text-white/50">Cập nhật</span>
                          <span className="text-xs text-white">{formatDate(data.updatedDate)}</span>
                        </div>
                      </div>
                    </section>
                  </div>
