...
import { useAddDoctorToClinic, useUpdateClinicDoctor } from "@/hooks/data/useClinicHooks";
...
export function ClinicDoctorModal({ isOpen, onClose, clinicId, initialData }: Props) {
  ...
  useMemo(() => {
    if (initialData) {
      ...
      setForm({ doctorId: "", specialty: "", consultationFee: "", status: "Active" });
    }
  }, [initialData, isOpen]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (initialData) {
        ...
        if (!form.doctorId) { alert("Vui lòng nhập ID bác sĩ"); return; }
          ...
          >
            ...
            <div className="flex items-center justify-between border-b border-gray-200 bg-white/5 p-6 dark:border-white/10">
              <h2 className="text-base font-semibold text-gray-900 dark:text-white">
                {initialData ? "Cập nhật Bác sĩ Phòng khám" : "Thêm Bác sĩ vào Phòng khám"}
              </h2>
            ...
            {/* Content */}
            <div className="p-6">
              <form id="clinic-doctor-form" onSubmit={handleSubmit} className="space-y-4">
                {!initialData && (
                  <Input
                    label="Doctor ID"
                ...
                {initialData && (
                  <div className="flex items-center gap-3 rounded-xl border border-gray-200 p-3 dark:border-white/10">
                    {initialData.doctorAvatar && (
                      <img src={initialData.doctorAvatar} className="h-10 w-10 rounded-full object-cover" />
                    )}
                    <div>
                      <p className="text-sm font-semibold text-gray-900 dark:text-white">{initialData.doctorName}</p>
                      <p className="text-xs text-gray-400">{initialData.doctorId.slice(0, 12)}...</p>
                    </div>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-4">
                  <Input label="Chuyên khoa" placeholder="VD: Nội khoa" value={form.specialty} onChange={(val) => setForm({ ...form, specialty: val })} />
                  <Input label="Giá khám (VNĐ)" type="number" placeholder="VD: 200000" value={form.consultationFee} onChange={(val) => setForm({ ...form, consultationFee: val })} />
                </div>

                {initialData && (
                  <div className="flex flex-col gap-1.5">
                    <p className="text-[13px] font-medium text-gray-700 dark:text-gray-200">Trạng thái</p>
                    <GlassSelect
                      value={form.status}
            ...
            <div className="flex justify-end gap-3 border-t border-gray-200 bg-white/5 p-6 dark:border-white/10">
              ...
              <button form="clinic-doctor-form" type="submit" disabled={isPending} className="rounded-lg bg-primary px-4 py-2 text-sm font-medium text-white transition hover:opacity-90 disabled:opacity-50">
                {isPending ? "Đang lưu..." : initialData ? "Cập nhật" : "Thêm bác sĩ"}
              </button>
            </div>
          </motion.div>
        </div>
