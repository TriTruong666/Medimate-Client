...
export function ConsultationSessionDetailModal({
  open,
  session,
  onClose,
}: Props) {
      ...
      {open && (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          ...
          <motion.div
            ...
            className="z-10 flex w-full max-w-3xl flex-col overflow-hidden rounded-2xl border border-gray-400 bg-white shadow-2xl transition-all duration-300 dark:border-white/10 dark:bg-neutral-900/90 dark:backdrop-blur-xl"
          >
            {/* Header */}
              ...
              <div className="space-y-4">
                ...
                <section className="overflow-hidden rounded-2xl border border-gray-400 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
                  ...
                  <div className="grid gap-6 p-5 sm:grid-cols-2 lg:grid-cols-4">
                    ...
                    <InfoItem
                      ...
                      label="Giám hộ"
                      value={
                        session.guardianUserId ? "Có liên kết" : "Không có"
                      }
                    />
                  </div>
                </section>
                ...
                <section className="overflow-hidden rounded-2xl border border-gray-400 bg-white shadow-sm dark:border-white/10 dark:bg-white/5">
                  ...
                  <div className="grid gap-2 p-4 sm:grid-cols-3">
                    <ParticipantItem
                      role="Bệnh nhân"
                      joined={session.userJoined}
                    />
                    <ParticipantItem
                      role="Bác sĩ"
                      joined={session.doctorJoined}
                    />
                    <ParticipantItem
                      role="Người giám hộ"
                      joined={session.guardianJoined}
                    />
                  </div>
                </section>
...
function InfoItem({
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      <p
        className={`text-sm font-semibold ${highlight ? "text-primary dark:text-primary-400" : "text-gray-900 dark:text-white"}`}
      >
        {value}
      </p>
    </div>
  ...
  return (
    ...
    >
      <div className="min-w-0">
        ...
        <p
          className={`mt-0.5 truncate text-sm font-semibold ${isJoined ? "text-emerald-700 dark:text-emerald-400" : "text-gray-400 dark:text-gray-600"}`}
        >
          {isJoined ? "Đã tham gia" : "Vắng mặt"}
        </p>
      </div>
      <div
        className={`h-2 w-2 rounded-full ${isJoined ? "bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]" : "bg-gray-300 dark:bg-gray-700"}`}
      />
    </div>
  );
}
