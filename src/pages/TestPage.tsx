...
function CompleteSection() {
  return (
    <div className="relative flex min-h-screen w-full items-center justify-center overflow-hidden bg-black px-6">
      ...
      <motion.div
        ...
        className="pointer-events-none absolute -z-10 h-[600px] w-[600px] rounded-full bg-white/5 blur-3xl"
      />
      ...
      <div className="flex max-w-3xl flex-col items-center text-center">
        ...
        <motion.h1
          ...
          className="text-5xl leading-tight font-semibold tracking-tight text-white md:text-7xl"
        >
          Hồ sơ của bạn
          <br />
          đang được xem xét.
        ...
        >
          Đội ngũ Medimate sẽ phản hồi trong vòng{" "}
          <span className="text-white">48 giờ</span>.
          <br />
          Chúng tôi sẽ thông báo ngay khi hoàn tất xác minh.
        </motion.p>
      </div>
    </div>
  );
  ...
  return (
    <div className="relative h-screen w-full font-sans antialiased">
      ...
      {setupProgress === "avatar" && (
        <SetupItem
          onClick={() => setSetupProgress("password")}
...
function AvatarUpload() {
  ...
  return (
    <div className="flex w-full flex-col items-center space-y-6">
      {!preview ? (
        <label className="group relative flex h-56 w-140 cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-white/20 bg-black transition-all duration-300 hover:border-white/40 hover:bg-white/[0.04]">
          <input
            type="file"
            accept="image/*"
            className="hidden"
          ...
          {/* subtle inner glow */}
          <div className="pointer-events-none absolute inset-0 rounded-2xl bg-[radial-gradient(circle_at_center,rgba(255,255,255,0.05),transparent_70%)] opacity-0 transition-opacity duration-300 group-hover:opacity-100" />
        ...
        <div className="group relative w-56">
          <img
            src={preview}
            alt="Avatar preview"
            className="h-56 w-full rounded-full object-cover ring-1 ring-white/10 transition-all duration-300 group-hover:ring-white/30"
          />

          {/* overlay */}
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-3 rounded-2xl bg-black/60 opacity-0 transition-opacity duration-300 group-hover:opacity-100">
            <label className="cursor-pointer rounded-full border border-white/20 px-5 py-2 text-xs tracking-wide text-white transition hover:border-white/50">
              Thay đổi
              <input
                type="file"
                accept="image/*"
...
function UploadCertificate() {
  ...
  return (
    <div className="flex w-full max-w-2xl flex-col space-y-6">
      {/* Upload Box */}
      <label className="group relative flex h-48 w-full cursor-pointer flex-col items-center justify-center rounded-2xl border border-dashed border-white/20 bg-black transition-all duration-300 hover:border-white/40 hover:bg-white/[0.04]">
        <input
          type="file"
          multiple
          accept=".pdf"
        ...
        <div className="relative flex flex-col items-center space-y-4 text-neutral-400 transition-all duration-300 group-hover:text-white">
          ...
          <div className="text-center">
            ...
            <p className="mt-1 text-xs text-neutral-500">
              Chỉ hỗ trợ PDF — tối đa 10MB mỗi file
            </p>
          </div>
        </div>
      </label>
