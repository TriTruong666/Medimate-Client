...
export default function ClinicDetailPage() {
  ...
  const [editingDoctor, setEditingDoctor] = useState<ClinicDoctorDto | null>(
    null,
  );
  ...
  const handleRemoveDoctor = async (clinicDoctorId: string) => {
    ...
    try {
      await removeDoctorMutation.mutateAsync(clinicDoctorId);
    } catch {}
  };
    ...
    <div className="page-layout space-y-8">
      ...
      <div>
        ...
        <div className="mt-2 flex items-center gap-4">
          <button
            onClick={() => navigate(PATHS.DASHBOARD.CLINIC)}
          ...
          <div className="flex items-center gap-3">
            {clinic?.logoUrl ? (
              ...
              <div className="bg-primary/10 text-primary flex h-12 w-12 items-center justify-center rounded-xl text-2xl">
                <TbBuildingHospital />
              </div>
            )}
            <div>
          ...
          {clinic?.isActive ? (
            <Badge type="success" value="Đang hoạt động" />
          ) : (
      ...
      {clinic && (
        <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
          {[
            ...
            <div
              key={item.label}
              className="rounded-xl border border-gray-400 p-4 dark:border-white/10"
            >
            ...
            >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      {row.doctorId.slice(0, 8).toUpperCase()}
                    </p>
                  </div>
                </div>
              </td>
              <td className="dark:border-border-dark border-r border-gray-400 p-4">
                <div className="flex flex-col gap-0.5 text-[13px]">
                  <span className="font-medium text-gray-800 dark:text-gray-200">
                    {row.specialty ?? "Chưa cập nhật"}
                  </span>
                  <span className="text-xs text-gray-400">
                    {row.consultationFee.toLocaleString("vi-VN")} VNĐ / lần
                  </span>
                </div>
              </td>
              <td className="dark:border-border-dark border-r border-gray-400 p-4 text-center">
                <Badge
                  ...
                  value={row.status === "Active" ? "Đang HĐ" : "Tạm dừng"}
                />
              </td>
                ...
                <div className="flex items-center justify-center gap-1">
                  ...
                  <Tooltip content="Gỡ khỏi phòng khám">
                    <IconAction
                      icon={<FiTrash2 />}
                      danger
                      onClick={() => handleRemoveDoctor(row.id)}
                    />
                  </Tooltip>
                </div>
              </td>
